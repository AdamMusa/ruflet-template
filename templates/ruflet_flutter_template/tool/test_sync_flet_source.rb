...
class SyncFletSourceTest < Minitest::Test
  def setup
    @tmp = Dir.mktmpdir("flet-sync-fixture-")
    ...
    overlay = {
      ...
      "patches" => { "lib/flet.dart" => [{ "before" => "export 'original.dart';\n", "after" => "export 'original.dart';\nexport 'embedded.dart';\n" }] },
      "preserved_files" => { "lib/embedded.dart" => Digest::SHA256.hexdigest("reviewed local transport\n") }
    }
    write(File.join(@template, "tool/flet_transport_overlay.json"), JSON.generate(overlay))
    @sync = FletSourceSync.new(template: @template, source: @source)
    @backups = []
  end
  ...
  def target(path, bytes)
    write(File.join(@template, "flet_packages/flet", path), bytes)
  end
  ...
  def test_tracks_source_blobs_with_overlay_binary_assets_and_no_caches
    ...
    assert_match(/verified/, FletSourceSync.new(template: @template).check)
    files = @sync.manifest.fetch("files")
    assert_equal "reviewed_transport_overlay", files.dig("lib/flet.dart", "classification")
    assert_equal "reviewed_template_addition", files.dig("lib/embedded.dart", "classification")
    assert_equal "\x00\xfffont".b, File.binread(File.join(@template, "flet_packages/flet/vendor/math/assets/font.ttf"))
    refute files.keys.any? { |path| path.include?(".dart_tool") || path.include?("build/") || path.end_with?("pubspec.lock") }
    assert_includes File.read(File.join(@template, "pubspec.yaml")), "flet_ads: user-owned"
  ...
  def test_initial_adoption_is_explicit_and_old_files_are_recoverable
    ...
    assert_raises(FletSourceSync::Error) { sync(initial: false) }
    sync
    assert_equal "old bytes", File.read(File.join(@backups.fetch(0), "lib/obsolete.dart"))
    refute File.exist?(File.join(@template, "flet_packages/flet/lib/obsolete.dart"))
  end
  ...
  def test_dirty_source_fails_before_writing
    ...
    refute File.exist?(File.join(@template, "flet_packages/flet/pubspec.yaml"))
  end
  ...
  def test_local_drift_blocks_check_and_later_sync_without_overwrite
    sync
    target("lib/flet.dart", "user work")
    assert_raises(FletSourceSync::Error) { @sync.check }
    assert_raises(FletSourceSync::Error) { sync }
    assert_equal "user work", File.read(File.join(@template, "flet_packages/flet/lib/flet.dart"))
  end
  ...
  def test_symlink_in_target_is_not_followed
    ...
    File.symlink(outside, File.join(@template, "flet_packages/flet/lib/unsafe.dart"))
    assert_raises(FletSourceSync::Error) { sync }
