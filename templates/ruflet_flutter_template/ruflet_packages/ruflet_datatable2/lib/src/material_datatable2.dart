...
import 'package:ruflet/ruflet.dart' as ft;
import 'package:ruflet/ruflet.dart';
import 'package:flutter/material.dart';

import 'utils/datatable.dart';

class MaterialDataTable2Control extends StatefulWidget {
  final Control control;

  const MaterialDataTable2Control({
    super.key,
    required this.control,
  });

  @override
  State<MaterialDataTable2Control> createState() => _DataTable2ControlState();
}

class _DataTable2ControlState extends State<MaterialDataTable2Control> {
  ...
  Widget build(BuildContext context) {
    ...
    var border = widget.control.getBorder("border", RufletStyleTheme.of(context));
    var borderRadius = widget.control.getBorderRadius("border_radius");
    var gradient =
        widget.control.getGradient("gradient", RufletStyleTheme.of(context));
    var horizontalLines = widget.control
        .getBorderSide("horizontal_lines", RufletStyleTheme.of(context));
    var verticalLines = widget.control
        .getBorderSide("vertical_lines", RufletStyleTheme.of(context));
    ...
    if (bgColor != null ||
      ...
      decoration = (defaultDecoration as BoxDecoration).copyWith(
          color: parseColor(bgColor, RufletStyleTheme.of(context)),
          border: border,
    ...
    var datatable2 = DataTable2(
      ...
      dataRowColor: widget.control
          .getWidgetStateColor("data_row_color", RufletStyleTheme.of(context)),
      dataRowHeight: widget.control.getDouble("data_row_height"),
      sortArrowIconColor:
          widget.control.getColor("sort_arrow_icon_color", context),
      dataTextStyle: widget.control
          .getTextStyle("data_text_style", RufletStyleTheme.of(context)),
      headingRowColor: widget.control
          .getWidgetStateColor("heading_row_color", RufletStyleTheme.of(context)),
      headingRowHeight: widget.control.getDouble("heading_row_height"),
      headingTextStyle: widget.control
          .getTextStyle("heading_text_style", RufletStyleTheme.of(context)),
      ...
      rows: widget.control.children("rows").map((row) {
        ...
        return DataRow2(
          ...
          color: row.getWidgetStateColor("color", RufletStyleTheme.of(context)),
