...
// This renderer fork is coupled to the pinned flutter_code_editor 0.3.5 controllers.
...
class CupertinoCodeField extends StatefulWidget {
  ...
  @override
  State<CupertinoCodeField> createState() => _CupertinoCodeFieldState();
}

class _CupertinoCodeFieldState extends State<CupertinoCodeField> {
  // Add a controller
  ...
  @override
  void didUpdateWidget(covariant CupertinoCodeField oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final defaultTextStyle = TextStyle(
      color: styles?[rootKey]?.color ?? themeData.textTheme.textStyle.color,
      fontSize: themeData.textTheme.textStyle.fontSize,
      height: themeData.textTheme.textStyle.height,
    );

    textStyle = defaultTextStyle.merge(widget.textStyle);

    final codeField = CupertinoTextField(
      focusNode: _focusNode,
    ...
    final editingField = LayoutBuilder(
  ...
  OverlayEntry _buildSearchOverlay() {
    final borderColor =
        _getTextColorFromTheme() ?? CupertinoColors.label.resolveFrom(context);
    return OverlayEntry(
      builder: (context) {
        return Positioned(
          bottom: 10,
