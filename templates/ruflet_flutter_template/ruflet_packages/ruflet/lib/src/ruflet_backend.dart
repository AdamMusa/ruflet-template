...
import 'ruflet_core_extension.dart';
...
/// RufletBackend - Handles business logic, provides data, and acts as ChangeNotifier
class RufletBackend extends ChangeNotifier {
  ...
  late final List<RufletExtension> extensions;
  ...
  RufletBackendChannel? _backendChannel;
  ...
  RufletBackend(
    ...
    this.extensions = [...extensions, RufletCoreExtension()];
    ...
    if (errorsHandler != null) {
      ...
      } else if (controlId != null && _parentRufletBackend != null) {
        // parent error handler
        errorsHandler?.addListener(() {
          _parentRufletBackend?.target?.triggerControlEventById(
  ...
  static RufletBackend of(BuildContext context) {
    return Provider.of<RufletBackend>(context, listen: false);
  }
  ...
  @override
  void dispose() {
    debugPrint("Disposing Ruflet backend.");
    _disposed = true;
  ...
  Future<void> connect() async {
    debugPrint("Connecting to Ruflet backend $pageUri...");
    try {
      _backendChannel = (channelBuilder ?? RufletBackendChannel.new)(
      ...
      debugPrint("Error connecting to Ruflet backend: $e");
      error = e.toString();
      _onDisconnect();
    }
  }
  ...
  _onClientRegistered(RegisterClientResponseBody resp) {
    if (resp.error?.isEmpty ?? true) {
      // all good!
      // if a root RufletApp, store session ID globally
      if (controlId == null) {
