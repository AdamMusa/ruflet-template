...
String getWebsocketEndpointPath(String uriPath) {
  return rufletJS?.webSocketEndpoint.trimSymbol("/") ?? "ws";
}

String getRufletRouteUrlStrategy() {
  return rufletJS?.routeUrlStrategy ?? "";
}

bool isPyodideMode() {
  return rufletJS?.pyodide == true;
}

bool isMultiView() {
  return rufletJS?.multiView == true;
}
...
@staticInterop
class RufletJS {
  external factory RufletJS();
}

extension RufletJSExtension on RufletJS {
...
@JS('ruflet')
external RufletJS? get rufletJS;
