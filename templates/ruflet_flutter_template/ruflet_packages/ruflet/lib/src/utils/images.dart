...
ColorFilter? parseColorFilter(dynamic value, RufletStyleTheme theme,
...
Widget buildImage({
  ...
  if (resolvedSrc.hasBytes) {
    ...
    if (stringSrc.contains(svgTag)) {
      ...
      var assetSrc = RufletBackend.of(context).getAssetSource(stringSrc);
      if (assetSrc.isFile) {
        // SVG File
...
extension ImageParsers on Control {
  ...
  ColorFilter? getColorFilter(String propertyName, RufletStyleTheme theme,
      [ColorFilter? defaultValue]) {
