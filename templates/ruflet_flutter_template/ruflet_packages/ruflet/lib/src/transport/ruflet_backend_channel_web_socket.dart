...
class RufletWebSocketBackendChannel implements RufletBackendChannel {
  late final String _wsUrl;
  late final bool _isLocalConnection;
  RufletBackendChannelOnMessageCallback onMessage;
  RufletBackendChannelOnDisconnectCallback onDisconnect;
  WebSocketChannel? _channel;

  RufletWebSocketBackendChannel(
  ...
  _onMessage(message) {
    onMessage(Message.fromList(
        msgpack.deserialize(message, extDecoder: RufletMsgpackDecoder())));
  }

  @override
  void send(Message message) {
    _channel?.sink.add(
        msgpack.serialize(message.toList(), extEncoder: RufletMsgpackEncoder()));
  }
