...
import '../widgets/platform_tooltip.dart';
...
Widget _tooltip(Widget widget, Control control) {
  final skipProps = control.internals?["skip_properties"] as List?;
  if (skipProps?.contains("tooltip") == true) return widget;

  return PlatformControlTooltip(control: control, child: widget);
}

Widget _badge(Widget widget, Control control) {
  final skipProps = control.internals?["skip_properties"] as List?;
  if (skipProps?.contains("badge") == true) return widget;

  return PlatformControlBadge(control: control, child: widget);
}
