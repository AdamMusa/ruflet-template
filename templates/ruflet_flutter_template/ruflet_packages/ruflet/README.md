...
Ruflet is a framework that allows building mobile, desktop and web applications
...
Beautiful UI widgets with Material and Cupertino design: layout, navigation, dialogs, charts - Ruflet uses Flutter to render UI.
...
Ruflet apps run natively in modern browsers using WebAssembly and Pyodide, with no server required. Prefer server-side? Deploy as a Python web app with real-time UI updates.
...
Test your project on your own mobile device with Ruflet App. See your app updates as you make changes.

### <img src="https://flet.dev/img/pages/home/extensible.svg" width="25" align="top" />&nbsp;&nbsp;&nbsp;Extensible

Easily wrap any of thousands of Flutter packages to use with Ruflet or build new controls in pure Python using built-in UI primitives.

### <img src="https://flet.dev/img/pages/home/accessible.svg" width="25" align="top" />&nbsp;&nbsp;&nbsp;Accessible

Ruflet is built with Flutter which has solid accessibility foundations on Android, iOS, web, and desktop.
...
To run the app, install `ruflet`:

```bash
pip install 'ruflet[all]'
```

then launch the app:

```bash
ruflet run counter.py
```
...
To run the same app as a web app use `--web` option with `ruflet run` command:

```bash
ruflet run --web counter.py
```
...
* [Email us](mailto:hello@ruflet.dev)

## Contributing

Want to help improve Ruflet? Check out the [contribution guide](https://docs.flet.dev/contributing).
