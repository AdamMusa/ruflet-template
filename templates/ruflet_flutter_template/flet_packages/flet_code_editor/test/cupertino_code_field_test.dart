...
import 'package:flet_code_editor/src/cupertino_editor/src/code_field/code_field.dart';
...
void main() {
  ...
  testWidgets(
      ...
      (tester) async {
    final controller =
        FletCodeController(text: 'alpha beta alpha', language: javascript);
    final focus = FocusNode();
    await tester.pumpWidget(
        host(CupertinoCodeField(controller: controller, focusNode: focus)));
    await tester.tap(find.byType(CupertinoTextField));
  ...
  testWidgets('Material field retains the upstream Material editor',
      (tester) async {
    final controller =
        FletCodeController(text: 'original', language: javascript);
