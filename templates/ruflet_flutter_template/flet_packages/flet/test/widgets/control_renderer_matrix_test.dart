...
class MatrixBackend extends FletBackend {
  MatrixBackend(TargetPlatform target)
      : super(
            pageUri: Uri.parse('flet://matrix'),
            assetsDir: '',
            extensions: [],
            multiView: false) {
    platform = target;
...
void main() {
  final registry = File('lib/src/flet_core_extension.dart')
      .readAsStringSync()
  ...
  const appShells = {'Page', 'View', 'FletApp'};
  ...
  for (final platform in [
    ...
    for (final type in types.difference(appShells)) {
      testWidgets(
        ...
        final rendered = FletCoreExtension().createWidget(null, root);
        ...
        if (design == PageDesign.cupertino) {
          ...
          // compatibility theme, even on Android. It is not a Flet renderer.
          expect(
