...
mixin SelectionManagerMixin<T extends StatefulWidget> on State<T>
  ...
  void copySelection(SelectionChangedCause cause) {
    if (!copyEnabled ||
        ...
        controller.selection.isCollapsed) return;
  ...
  void bringIntoView(TextPosition position) {
    ...
    rootRenderBox.showOnScreen(
        rect: Rect.fromLTWH(endpoint.dx, 0, 1, rootRenderBox.size.height));
  }

  @override
  void userUpdateTextEditingValue(
      TextEditingValue value, SelectionChangedCause cause) {
    // Display math is read-only. The legacy delegate value maps only select-all.
