...
struct RufletSubtitleConfiguration {
  ...
  init(_ value: RufletValue?) {
    ...
    background = parseColor(style["bgcolor"]?.text ?? style["background_color"]?.text, Color.black.opacity(2 / 3))
...
func makeVideoItem(_ media: RufletVideoMedia, control: RufletControl) throws -> AVPlayerItem {
  ...
  let options: [String: Any] = media.httpHeaders.isEmpty
    ? [:]
...
private func subtitleTime(_ value: String) -> TimeInterval? {
  let timestamp = value.trimmingCharacters(in: .whitespaces).split(separator: " ").first.map(String.init) ?? ""
