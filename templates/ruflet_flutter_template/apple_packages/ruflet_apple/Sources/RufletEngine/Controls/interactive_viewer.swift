...
public struct InteractiveViewerControl: View {
    ...
    private func viewer(_ content: RufletControl) -> some View {
            ...
            ControlWidget(control: content)
                ...
                .background {
                    GeometryReader { childProxy in
                        Color.clear.preference(key: RufletInteractiveContentSizeKey.self, value: childProxy.size)
                    }
                }
        ...
        return AnyView(view.clipped(antialiased: clipBehavior == "antialias" || clipBehavior == "antialiaswithsavelayer"))
    }
    ...
    private var interactionGesture: some Gesture {
        ...
        .onEnded { value in
            ...
            if !control.disabled {
                ...
                control.triggerEvent("interaction_end", data: [
                    "pc": .int(0),
    ...
    private func triggerStart(global: CGPoint, magnification: Double) {
        control.triggerEvent("interaction_start", data: [
    ...
    private func clampedScale(_ proposed: Double) -> Double {
        ...
        if constrained, contentSize.width > 0, contentSize.height > 0 {
            minimum = max(minimum, max(viewportSize.width / contentSize.width, viewportSize.height / contentSize.height))
        }
    ...
    private func clampedOffset(_ proposed: CGSize, at scale: Double) -> CGSize {
        ...
        let maxX = max((contentSize.width * scale - viewportSize.width) / 2, 0) + margin.leading + margin.trailing
        let maxY = max((contentSize.height * scale - viewportSize.height) / 2, 0) + margin.top + margin.bottom
        return CGSize(
    ...
    private var inertiaDuration: TimeInterval {
        let friction = control.number("interaction_end_friction_coefficient", default: 0.0000135) ?? 0.0000135
