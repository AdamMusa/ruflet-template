...
public struct ButtonControl: View {
    ...
    private var buttonStyle: RufletAppleButtonStyle {
        let details = rufletDictionary(control.internals?["style"].map(rufletAny) ?? control.dynamicValue("style"))
        let foreground = parseColor(details?["color"] as? String)
            ?? parseColor(control.string("color"))
            ?? .accentColor
        let background: Color
        switch variant {
        case .filled: background = parseColor(details?["bgcolor"] as? String) ?? parseColor(control.string("bgcolor")) ?? .accentColor
        case .tonal: background = parseColor(details?["bgcolor"] as? String) ?? .accentColor.opacity(0.18)
        case .outlined, .text: background = parseColor(details?["bgcolor"] as? String) ?? .clear
        case .elevated: background = parseColor(details?["bgcolor"] as? String) ?? .rufletSystemBackground
        }
        let border = variant == .outlined
            ? (parseBorderSide(details?["side"], defaultColor: foreground) ?? RufletBorderSide(width: 1, color: foreground))
            : parseBorderSide(details?["side"], defaultColor: foreground)
        let radius = parseBorderRadius(details?["shape"] ?? control.dynamicValue("shape"))?.uniform ?? 20
