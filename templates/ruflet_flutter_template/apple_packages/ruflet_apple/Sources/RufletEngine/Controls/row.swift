...
public struct RowControl: View {
    ...
    private var content: AnyView {
        ...
        return AnyView(HStack(alignment: verticalAlignment, spacing: spacing) {
            ForEach(Array(control.children("controls").enumerated()), id: \.element.id) { _, child in
                ControlWidget(control: child)
            }
        }
        .frame(maxWidth: control.boolean("tight", default: false) ? nil : .infinity, alignment: horizontalAlignment))
    }
