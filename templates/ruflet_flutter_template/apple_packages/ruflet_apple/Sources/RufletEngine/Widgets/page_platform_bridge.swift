...
struct RufletPageLifecycleMonitor<Content: View>: View {
  ...
  var body: some View {
      ...
      content()
        ...
        ) { _ in onTransition("resume") }
        .onReceive(
          NotificationCenter.default.publisher(for: UIApplication.willResignActiveNotification)
        ) { _ in onTransition("inactive") }
        .onReceive(
        ...
        ) { _ in onTransition("detach") }
    #elseif os(macOS)
      content()
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.didUnhideNotification)) {
          _ in onTransition("show")
        }
        .onReceive(
          NotificationCenter.default.publisher(for: NSApplication.didBecomeActiveNotification)
        ) { _ in onTransition("resume") }
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.didHideNotification)) {
          _ in onTransition("hide")
        }
        .onReceive(
  ...
  struct RufletPagePlatformBridge: UIViewRepresentable {
    ...
    func updateUIView(_ view: UIView, context: Context) {
      RufletPageCaptureRegistry.register(backend: control.backend) { [weak view] pixelRatio in
        ...
        return renderer.image { _ in root.drawHierarchy(in: root.bounds, afterScreenUpdates: true) }
          .pngData()
      }
    }
...
struct RufletPageKeyboardMonitor: View {
  ...
  private final class RufletKeyboardViewController: UIViewController {
    ...
    override var keyCommands: [UIKeyCommand]? {
      ...
      commands.append(
        UIKeyCommand(input: UIKeyCommand.inputEscape, modifierFlags: [], action: #selector(back)))
      return commands
    }
